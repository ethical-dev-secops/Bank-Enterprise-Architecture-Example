...
namespace BankAccountManagementSystem.Infrastructure.Repositories
{
    ...
    public class AddressRepository : RepositoryBase<Address, Address, ApplicationDbContext>, IAddressRepository
    {
        ...
        public async Task<Address?> FindByIdAsync(long id, CancellationToken cancellationToken = default)
        {
            return await FindAsync(x => x.Id == id, cancellationToken);
        }

        public async Task<List<Address>> FindByIdsAsync(long[] ids, CancellationToken cancellationToken = default)
        {
