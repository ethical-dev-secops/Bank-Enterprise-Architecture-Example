...
namespace BankAccountManagementSystem.Infrastructure.Repositories
{
    ...
    public class AccountHolderRepository : RepositoryBase<AccountHolder, AccountHolder, ApplicationDbContext>, IAccountHolderRepository
    {
        ...
        public async Task<AccountHolder?> FindByIdAsync(long id, CancellationToken cancellationToken = default)
        {
            return await FindAsync(x => x.Id == id, cancellationToken);
        }

        public async Task<List<AccountHolder>> FindByIdsAsync(long[] ids, CancellationToken cancellationToken = default)
        {
