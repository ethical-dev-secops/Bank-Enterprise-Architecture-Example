...
namespace BankAccountManagementSystem.Domain.Repositories
{
    ...
    public interface IAddressRepository : IEFRepository<Address, Address>
    {
        ...
        Task<Address?> FindByIdAsync(long id, CancellationToken cancellationToken = default);
        [IntentManaged(Mode.Fully)]
        Task<List<Address>> FindByIdsAsync(long[] ids, CancellationToken cancellationToken = default);
    }
}